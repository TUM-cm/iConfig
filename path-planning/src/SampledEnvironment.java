...
public class SampledEnvironment {
    ...
    public SampledEnvironment(ArrayList<MapEntry> data, Point start) {
        super();
        ...
        //sample points within convex hull 150 5 |  25  5
        // chair 50  2 |  12  1
        for(double x=0.0; x<50.0; x=x+2.0){
            for(double y=0.0; y<12.0; y=y+1.0){
                data.add(new MapEntry(2,x,y,0.0,0.0,"",0d));
            }
        }
        ...
        for(MapEntry mapEntry: data) {
            switch(mapEntry.getClassCategory()) {
                ...
                case 2:
                    ...
                    Point samplePoint = new Point(i,mapEntry.getLocationx1(),mapEntry.getLocationy1());
                    i++;
                    if (!isDoublePoint(samplePoint,points)) {
                        points.add(samplePoint);
                    }

                    for(MapEntry complement_point: data) {
                        Point otherPoint = new Point(-1,complement_point.getLocationx1(),complement_point.getLocationy1());
                        if((samplePoint.getX() != otherPoint.getX()) || (samplePoint.getY() != otherPoint.getY())){
                            Edge edge = new Edge(samplePoint,otherPoint);
                            edges.add(edge);
                        }
                    }
                    break;
